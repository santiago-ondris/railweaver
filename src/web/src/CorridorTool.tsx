...
const distance = (value: number) =>
  value >= 1000 ? `${number(value / 1000, 2)} km` : meters(value)
  ...
  return (
    <aside className="corridor-tool" aria-label="Corredor candidato">
      ...
      <dl className="detail-list">
        <div>
          <dt>Origen</dt>
          <dd>{origin ? pointLabel(origin) : 'Marcá un punto en el mapa'}</dd>
        </div>
        <div>
          <dt>Destino</dt>
          <dd>{destination ? pointLabel(destination) : 'Marcá un punto en el mapa'}</dd>
        </div>
      </dl>
      {pointError && (
        <p className="corridor-validation" role="alert">
      ...
      {result?.status === 'no_feasible_path' && (
        <CorridorAlert
          title={`No existe un corredor con pendiente ≤ ${number(limit)} ‰ entre estos puntos`}
  ...
  return (
    <aside className="detail-panel" aria-label="Detalle del corredor">
      ...
      <p className="detail-provenance">
        Límite {number(metric.maxGradientLimitPermille)} ‰ · malla {search.gridStepMeters} m
      </p>
      <dl className="detail-list">
        ...
        <Metric
          ...
          value={`${number(metric.maxGradientPermille)} ‰ ≤ ${number(metric.maxGradientLimitPermille)} ‰`}
        />
        <Metric label="Subida" value={meters(metric.ascentMeters)} />
        <Metric label="Bajada" value={meters(metric.descentMeters)} />
        <Metric label="Cota mínima" value={meters(metric.minElevationMeters)} />
        <Metric label="Cota máxima" value={meters(metric.maxElevationMeters)} />
        <Metric label="Corte implícito máx." value={meters(metric.maxCutMeters)} />
        <Metric label="Relleno implícito máx." value={meters(metric.maxFillMeters)} />
  ...
  return (
    <section className="analysis-dock" aria-label="Perfil longitudinal del corredor">
      <div>
        ...
        <p className="data">
          Máx. {number(corridor.metrics.maxGradientPermille)} ‰ ≤ límite{' '}
          {number(corridor.metrics.maxGradientLimitPermille)} ‰
        </p>
        {samples.some((sample) => sample.elevationMeters === null) && (
          <p className="detail-hint">Hay tramos de terreno sin dato.</p>
        )}
      </div>
      <svg
...
function pointLabel(point: Coordinate) {
  return `${number(point.latitude, 5)}, ${number(point.longitude, 5)}`
}
