...
export function GeographicViewer({ region, railway, onTerrainStatus }: GeographicViewerProps) {
  ...
  const corridorPointsRef = useRef<{ origin: Coordinate | null; destination: Coordinate | null }>({ origin: null, destination: null })
    ...
    try {
      ...
      interaction.setInputAction((event: ScreenSpaceEventHandler.PositionedEvent) => {
        ...
        if (corridorActiveRef.current) {
          ...
          const next = corridorPointsRef.current.destination || !corridorPointsRef.current.origin
            ? { origin: coordinate, destination: null } : { ...corridorPointsRef.current, destination: coordinate }
          corridorPointsRef.current = next
          setCorridorOrigin(next.origin); setCorridorDestination(next.destination)
          setCorridorError(null); setCorridorSearchResult(null)
          return
        }
        const picked = scene.pick(event.position) as { id?: unknown } | undefined
        if (corridorLayerRef.current?.isPicked(picked?.id) || corridorEntitiesRef.current.includes(picked?.id as Entity)) {
      ...
      const keyboard = (event: KeyboardEvent) => {
        if (event.key === 'Escape' && corridorActiveRef.current) {
          corridorActiveRef.current = false; corridorRequestRef.current?.abort()
          corridorPointsRef.current = { origin: null, destination: null }
          setCorridorOrigin(null); setCorridorDestination(null); setCorridorActive(false); setCorridorBusy(false)
          setCorridorError(null); setCorridorSearchResult(null)
          return
        }
      ...
      return () => {
        ...
        corridorLayerRef.current?.destroy(); corridorLayerRef.current = null
  ...
  useEffect(() => {
    ...
    for (const [name, point] of [['Origen', corridorOrigin], ['Destino', corridorDestination]] as const) {
      if (!point) continue
      corridorEntitiesRef.current.push(viewer.entities.add({
        name, position: Cartesian3.fromDegrees(point.longitude, point.latitude),
        point: { pixelSize: 10, color: tokenColor('primary'), outlineColor: tokenColor('map-ground'), outlineWidth: 2 },
        label: { text: name, font: `${getComputedStyle(document.documentElement).getPropertyValue('--rw-text-body-size')} ${getComputedStyle(document.documentElement).getPropertyValue('--rw-font-ui')}`,
          fillColor: tokenColor('primary'), showBackground: true, backgroundColor: tokenColor('map-ground') },
      }))
    }
    viewer.scene.requestRender()
  ...
  const beginProfile = () => {
    corridorActiveRef.current = false; corridorRequestRef.current?.abort(); setCorridorActive(false); setCorridorBusy(false)
    setCorridorShown(false)
    setCorridorOrigin(null); setCorridorDestination(null)
    corridorPointsRef.current = { origin: null, destination: null }
  ...
  const beginCorridor = () => {
    profileActiveRef.current = false; setProfileActive(false)
    setProfile(null); setProfileError(null); setCorridorShown(false)
    ...
    corridorActiveRef.current = true; setCorridorActive(true)
    corridorPointsRef.current = { origin: null, destination: null }
    setCorridorOrigin(null); setCorridorDestination(null); setCorridorError(null); setCorridorSearchResult(null)
  }
  const cancelCorridor = () => {
    corridorRequestRef.current?.abort(); corridorRequestRef.current = null
    corridorActiveRef.current = false; corridorPointsRef.current = { origin: null, destination: null }
    setCorridorActive(false); setCorridorOrigin(null); setCorridorDestination(null)
    setCorridorError(null); setCorridorSearchResult(null); setCorridorBusy(false)
  }
  const generateCorridor = async () => {
    if (!corridorOrigin || !corridorDestination) return
    const controller = new AbortController()
    corridorRequestRef.current = controller
    setCorridorBusy(true); setCorridorError(null); setCorridorSearchResult(null)
    try {
      const result = await fetchCorridor(region.id, corridorOrigin, corridorDestination, Number(corridorGradient), controller.signal)
      if (controller.signal.aborted) return
      if (result.status === 'found') {
        setCorridorResult(result)
        corridorSerialRef.current++
        setCorridorNumber(corridorSerialRef.current)
        setCorridorShown(true); setCorridorActive(false); corridorActiveRef.current = false
      } else {
        setCorridorSearchResult(result)
      }
    } catch (error) {
      if (!controller.signal.aborted) setCorridorError(error instanceof Error ? error.message : 'Error de red.')
    } finally {
      if (corridorRequestRef.current === controller) { corridorRequestRef.current = null; setCorridorBusy(false) }
    }
  }
  ...
  return <>
    <aside className="side-rail" aria-label="Control de capas">
      ...
      <button type="button" className="button-secondary" aria-pressed={corridorActive} disabled={terrainUnavailable}
        title={terrainUnavailable ? 'Requiere el dataset de relieve' : undefined} onClick={beginCorridor}>Corredor</button>
      {profileActive && <p className="tool-hint">Marcá vértices. Doble clic o Enter para terminar; Esc para cancelar.</p>}
    </aside>
    <section className="map-stage" aria-label={`Visor geográfico de ${region.name}`}><div ref={containerRef} className="map-canvas" />
      {corridorActive && <CorridorTool origin={corridorOrigin} destination={corridorDestination} gradient={corridorGradient}
        setGradient={setCorridorGradient} bounds={region.boundingBox} busy={corridorBusy} result={corridorSearchResult}
        error={corridorError} onGenerate={() => { void generateCorridor() }} onCancel={cancelCorridor} />}</section>
    {corridorShown && corridorResult?.corridor
      ? <CorridorDetail corridor={corridorResult.corridor} search={corridorResult.search} id={corridorNumber}
        onRemove={() => { setCorridorResult(null); setCorridorShown(false); setCorridorOrigin(null); setCorridorDestination(null) }} />
    ...
    {corridorShown && corridorResult?.corridor ? <CorridorProfile corridor={corridorResult.corridor} />
